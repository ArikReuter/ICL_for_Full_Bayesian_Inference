...
class EvaluateRealWorld_DifferentDatasetPosteriorModel(Evaluate):
    """
    A class to evaluate the performance of different models on real world data.
    Use a slightly augment version for the posterior model
    """
    ...
    def __init__(
            ...
            evaluation_dataset_posterior_model: list[dict],
    ...
    def sample_posterior_model(self, model: PosteriorComparisonModel) -> list[dict]:
        ...
        posterior_samples = []
        for case in tqdm(self.evaluation_dataset_posterior_model):
    ...
    def sample_comparison_model(self, model: PosteriorComparisonModel) -> list[dict]:
        """
        sample the comparison models with their datasets
        Args:
            model: PosteriorComparisonModel: the model to sample from
        Returns:
            list[dict]: a list of dictionaries containing the samples
        """

        posterior_samples = []
        for case in tqdm(self.evaluation_list):
            data = self.results_dict_to_data_for_model(case)
            samples = model.sample_posterior(*data)
    ...
    def _run_eval_raw_results(self) -> tuple:
        ...
        comparison_model_samples = [self.sample_comparison_model(model) for model in self.comparison_models]
