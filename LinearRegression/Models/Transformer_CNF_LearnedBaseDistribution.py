...
class TransformerPlusNLP(torch.nn.Module):
    """
    A transformer encoder plus an mlp head to process the output of the transformer encoder
    for the time-conditioning of the decoder
    """
    ...
    def __init__(
        ...
        """
        Args:
            transformer_encoder: Transformer: the transformer encoder
            mlp: torch.nn.Module: the mlp to process the output of the transformer encoder
        """
        super(TransformerPlusNLP, self).__init__()
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        ...
        Args:
            x: torch.tensor: the input tensor of shape (n_batch_size, seq_len, n_features). If self.transpose_input is True, the input tensor should have the shape (n_batch_size, n_features, seq_len)
        Returns:
            torch.tensor: the output tensor of shape (n_batch_size, n_embedding_dim)
        """
        x = self.transformer_encoder(x)
        x = torch.mean(x, dim=1)  # average the output of the transformer encoder over the sequence length
        x = self.mlp(x)  # process the output of the transformer encoder with an MLP

        return x
    

class TransformerCNFConditionalDecoderDouble_parallel_learnedBaseDistribution2(torch.nn.Module):
    """
    A transformer model that uses a pre-trained encoder to provide samples from the base distribution
    Usage is indented as follows: 
    ...
    def __init__(self,
                 encoder: TransformerReturnRepresentations,
                 model_posterior: ModelPosterior,
                 model_to_process_encoder_output: torch.nn.Module,
                 mlp_to_process_time_conditioning: torch.nn.Module,
                 decoder: TransformerDecoderConditionalDouble_parallel,
                 mlp_to_process_decoder_output: MLPConditionalDouble_parallel,
                 freeze_encoder: bool = True
    ...
    def forward_decoder(self, z: torch.tensor, x_encoder: torch.tensor, condition_time: torch.tensor) -> torch.tensor:
            ...
            x_encoder_processed = self.model_to_process_encoder_output(x_encoder) # process the output of the encoder with an MLP
