...
def make_lm_program_ig_weird2_batched(
        tau: float = 1.0,
        a: float = 5.0,
        b: float = 2.0
    ) -> 'LM_abstract.pprogram_linear_model_return_dict':
        ...
        def multivariate_lm_return_dict(x: torch.Tensor, y: torch.Tensor = None) -> dict:
                ...
                with pyro.plate("batch", batch_size, dim=-1):   
                        ...
                        beta = 0.5*(torch.sgn(4*beta))*(torch.abs(4*beta)+1)**(0.5)
...
def make_lm_program_ig_weird3_batched(
        tau: float = 1.0,
        a: float = 5.0,
        b: float = 2.0
    ) -> 'LM_abstract.pprogram_linear_model_return_dict':
...
def make_lm_program_ig_skewedNormal_batched(
        tau: float = 1.0,
        a: float = 5.0,
        b: float = 2.0,
        omega: float = 1.0,
        ...
        Args: 
                ...
                a: float: the shape parameter of the gamma prior on sigma_squared
                b: float: the rate parameter of the gamma prior on sigma_squared
                omega: scale parameter of the skewed normal distribution
                delta: shape parameter of the skewed normal distribution introducing skewness
        Returns:
                LM_abstract.pprogram_linear_model_return_dict: a linear model probabilistic program
        """
        def multivariate_lm_return_dict(x: torch.Tensor, y: torch.Tensor = None) -> dict:
...
def make_lm_program_ig_skewedNormal(
        tau: float = 1.0,
        a: float = 5.0,
        b: float = 2.0,
        omega: float = 1.0,
